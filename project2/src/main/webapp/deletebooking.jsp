...
if (u_idParam != null) {
    ...
    try {
        ...
        PreparedStatement ps = con.prepareStatement("SELECT * FROM information WHERE u_id= ?");
...
<form method="post">
    ...
    <div>
        UserId: <%=u_id %><br>
        <input type="text" name="u_name" value="<%= u_name %>" placeholder="Enter Your UserName"><br>
...
<%
    ...
    out.println("<div>User ID not provided</div>");
}
    ...
    if ("Update".equals(action)) {
        ...
        if (u_name != null && !u_name.isEmpty()) {
            ...
            try {
                ...
                Class.forName("com.mysql.cj.jdbc.Driver");
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/bus_reservation_system","root", "SHREYASRAO123");
                PreparedStatement ps = con.prepareStatement("UPDATE information SET u_name=?,email=?,phone=?,password=? WHERE u_id=?");
                ...
                if (rowsAffected > 0) {
                    out.println("<div>Data updated successfully.</div>");
                } else {
                    out.println("<div>Failed to update data.</div>");
                }
                // Close resources
                ps.close();
                con.close();
            ...
            out.println("<div>Username cannot be empty.</div>");
        }
        ...
        try {
            ...
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/bus_reservation_system","root", "SHREYASRAO123");
            PreparedStatement ps = con.prepareStatement("DELETE FROM information WHERE u_id=?");
            ps.setInt(1, Integer.parseInt(u_idParam));
            int rowsAffected = ps.executeUpdate();
            // Check if deletion was successful
            if (rowsAffected > 0) {
                out.println("<div>Record deleted successfully.</div>");
            } else {
                out.println("<div>Failed to delete record.</div>");
            }
            // Close resources
            ps.close();
            con.close();
